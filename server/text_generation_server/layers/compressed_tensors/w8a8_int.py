...
class W8A8IntLoader(WeightsLoader):
    ...
    def get_multi_weights_col(self, weights: "Weights", prefixes: List[str], dim: int):
        ...
        if self.load_input_scale:
            input_scale = [
                weights.get_tensor(f"{p}.input_scale", shape, to_dtype=False)
                for p, shape in zip(prefixes, shapes)
...
def _get_tensor_or_else(weights: Weights, prefix: str, other: Optional[torch.Tensor]):
        ...
        *,
        ...
        if input_scale is not None:
            if input_zero_point is None:
                ...
                input_zero_point = torch.clamp(
                    input_zero_point, int8_info.min, int8_info.max
                )
                input_zero_point = input_zero_point.to(torch.int32)
        ...
        self.zero_point_adj = (
            self.weight.sum(dim=0, keepdim=True, dtype=torch.int32)
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        if self.input_symmetric:
            ...
            assert (
                ...
                and input_zero_point is not None
            )

            if input_zero_point.numel() == 1:
                input_zero_point = input_zero_point.repeat(input.shape[0], 1)
